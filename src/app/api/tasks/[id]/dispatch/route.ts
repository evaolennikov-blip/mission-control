...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Link task_id to the session so we can track which task the session is serving
    if (session.task_id !== id) {
      run('UPDATE openclaw_sessions SET task_id = ?, updated_at = ? WHERE id = ?', [id, now, session.id]);
    }
